...
class ProjectTest < Minitest::Test
  ...
  def verify_response(expected, project)
    assert project.is_a?(Copper::Project)
  ...
  def setup
    ...
    @single_resource_url = get_uri(Copper::Project.api_name, @id)
    @create_url = get_uri(Copper::Project.api_name)
  end

  def test_project_get
    stub_request(:get, @single_resource_url).with(headers: headers).to_return(status: 200, body: project_payload)

    project = Copper::Project.find(@id)
  ...
  def test_project_create
    ...
    project = Copper::Project.create(project_details)
    verify_response(project_details, project)
  end
  ...
  def test_project_update
    ...
    project = Copper::Project.new(initial_attributes)
    assert_equal initial_attributes[:name], project.name

    response = Copper::Project.update(project, project_details)
    verify_response(project_details, project)
  end
  ...
  def test_project_delete
    ...
    project = Copper::Project.new(project_details)
    response = Copper::Project.delete(project.id)
